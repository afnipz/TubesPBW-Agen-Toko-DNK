...
// SignUpPage menampilkan halaman sign up
func SignUpPageAdmin(w http.ResponseWriter, r *http.Request) {
	fp := filepath.Join("views", "signupAdmin.html")
	tmpl, err := template.ParseFiles(fp)
	if err != nil {
		http.Error(w, "Unable to load sign up page", http.StatusInternalServerError)
		return
...
// SignUpHandler memproses form sign up
func SignUpHandlerAdmin(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost {
			err := r.ParseForm()
			if err != nil {
			...
			// Check if username already exists
			var exists bool
			err = db.QueryRow("SELECT EXISTS(SELECT 1 FROM admin WHERE username = ?)", username).Scan(&exists)
			if err != nil {
				http.Error(w, "Internal server error", http.StatusInternalServerError)
				return
			}
...
// CheckUsernameHandler untuk memeriksa apakah username sudah ada
func CheckUsernameHandlerAdmin(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		username := r.URL.Query().Get("username")
		var exists bool
		err := db.QueryRow("SELECT EXISTS(SELECT 1 FROM admin WHERE username = ?)", username).Scan(&exists)
...
// LoginPage menampilkan halaman login
func LoginPageAdmin(w http.ResponseWriter, r *http.Request) {
	fp := filepath.Join("views", "loginAdmin.html")
	tmpl, err := template.ParseFiles(fp)
	if err != nil {
		http.Error(w, "Unable to load login page", http.StatusInternalServerError)
		return
...
// LoginHandler memproses form login
func LoginHandlerAdmin(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost {
			err := r.ParseForm()
			if err != nil {
			...
			var hashedPassword string
			err = db.QueryRow("SELECT password FROM admin WHERE username = ?", username).Scan(&hashedPassword)
			if err != nil {
				if err == sql.ErrNoRows {
					http.Error(w, "Invalid username or password", http.StatusUnauthorized)
					return
			...
			http.Redirect(w, r, "/LandingPage/admin", http.StatusSeeOther)
		} else {
			http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		}
	}
